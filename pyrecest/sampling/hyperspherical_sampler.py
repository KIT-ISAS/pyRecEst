...
from pyrecest.distributions import HypersphericalUniformDistribution, AbstractSphericalDistribution
import itertools
...
def get_grid_hypersphere(method: str, grid_density_parameter: int):
    if method == 'healpix':
        samples, grid_specific_description = HealpixSampler().get_grid(grid_density_parameter)
    elif method == 'driscoll_healy':
        samples, grid_specific_description = DriscollHealySampler().get_grid(grid_density_parameter)
    elif method in ('fibonacci', 'spherical_fibonacci'):
        samples, grid_specific_description = SphericalFibonacciSampler().get_grid(grid_density_parameter)
    elif method == 'healpix_hopf':
        samples, grid_specific_description = HealpixHopfSampler().get_grid(grid_density_parameter)
    else:
...
class AbstractSphericalUniformSampler(AbstractHypersphericalUniformSampler):
    def sample_stochastic(self, n_samples: int, dim: int = 2): # Only having dim there for interface compatibility
        assert dim == 2
...
class AbstractSphericalCoordinatesBasedSampler(AbstractSphericalUniformSampler):
    @abstractmethod
    def get_grid_spherical_coordinates(self, grid_density_parameter: int) -> tuple[np.ndarray, np.ndarray, dict]:
        raise NotImplementedError()
    
    @beartype
    def get_grid(self, grid_density_parameter: int) -> tuple[np.ndarray, dict]:
        phi, theta, grid_specific_description = self.get_grid_spherical_coordinates(grid_density_parameter)
...
class HealpixSampler(AbstractHypersphericalUniformSampler):
    ...
    def get_grid(self, grid_density_parameter: int) -> tuple[np.ndarray, dict]:
        ...
        grid_specific_description = {"scheme": "healpix", "n_side": grid_density_parameter}
...
class DriscollHealySampler(AbstractSphericalCoordinatesBasedSampler):
    ...
    def get_grid_spherical_coordinates(self, grid_density_parameter: int) -> tuple[np.ndarray, np.ndarray, dict]:
        ...
        phi_theta_stacked_deg = np.array(list(itertools.product(phi_deg_mat, theta_deg_mat)))
        phi_theta_stacked_rad = np.radians(phi_theta_stacked_deg)
        
        phi = phi_theta_stacked_rad[:, 0]
        theta = phi_theta_stacked_rad[:, 1]
        
        grid_specific_description = {"scheme": "driscoll_healy", "l_max": grid_density_parameter,
                                     "n_lat": grid.nlat, "n_lon": grid.nlon}
...
class SphericalFibonacciSampler(AbstractSphericalCoordinatesBasedSampler):
    ...
    def get_grid_spherical_coordinates(self, grid_density_parameter: int) -> tuple[np.ndarray, np.ndarray, dict]:
        ...
        grid_specific_description = {"scheme": "spherical_fibonacci", 'n_samples': grid_density_parameter}
        return phi, theta, grid_specific_description
...
class AbstractHopfBasedS3Sampler(AbstractHypersphericalUniformSampler):
    @staticmethod
    @beartype
    def hopf_coordinates_to_quaterion_yershova(θ: np.ndarray, ϕ: np.ndarray, ψ: np.ndarray):
        """
        ...
        for i in range(grid_density_parameter[0] + 1):
            ...
            for j in range(len(healpix_points)):
                for k in range(len(psi_points)):
                    temp = np.array([healpix_points[j, 0], healpix_points[j, 1], psi_points[k]])
                    s3_points_list.append(temp)
                    
        s3_points = np.vstack(s3_points_list) # Need to stack like this and unpack
        grid = AbstractHopfBasedS3Sampler.hopf_coordinates_to_quaterion_yershova(s3_points[:, 0], s3_points[:, 1], s3_points[:, 2])
        
        grid_specific_description = {"scheme": "healpix_hopf", "layer-parameter": grid_density_parameter}
        return grid, grid_specific_description
