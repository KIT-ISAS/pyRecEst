...
class _Weights:
    ...
    def __init__(self, d: int, alpha: float):
        m = (alpha**2 - 1) * d
        self.sqrt_d_lambda = np.sqrt(d + m)
        self.wj = 1.0 / (2.0 * (d + m))
        self.wm = m / (m + d)
...
class UKFOnManifolds(AbstractFilter):
    ...
    >>> ukf.predict(omega=np.zeros(2), dt=1.0)
    >>> ukf.update(y=np.array([1.0, 0.5]))
    """
    ...
    def __init__(
        ...
        alpha_arr = np.broadcast_to(np.asarray(alpha, dtype=float), (3,)).copy()
        self._w_d = _Weights(self.d, alpha_arr[0])  # propagation / state
        self._w_q = _Weights(self.q, alpha_arr[1])  # propagation / noise
        self._w_u = _Weights(self.d, alpha_arr[2])  # update
    ...
    def predict(self, omega=None, dt: float = 1.0):
        ...
        """
        P = self._P + self.TOL * np.eye(self.d)

        w_zero = np.zeros(self.q)
        ...
        w_d = self._w_d
        chol_P = np.linalg.cholesky(P).T  # rows = columns of upper Chol.
        ...
        xi_mean = w_d.wj * np.sum(new_xis, axis=0)
        new_xis_centered = new_xis - xi_mean
        new_P = w_d.wj * new_xis_centered.T @ new_xis_centered + w_d.w0 * np.outer(
            xi_mean, xi_mean
        )

        # 3. Covariance contribution from noise
        w_q = self._w_q
        new_xis_q = np.zeros((2 * self.q, self.d))
        ...
        xi_mean_q = w_q.wj * np.sum(new_xis_q, axis=0)
        new_xis_q_centered = new_xis_q - xi_mean_q
        Q_contrib = w_q.wj * new_xis_q_centered.T @ new_xis_q_centered + w_q.w0 * np.outer(
            xi_mean_q, xi_mean_q
        )
    ...
    def update(self, y: np.ndarray):
        ...
        w_u = self._w_u
        chol_P = np.linalg.cholesky(P).T
        xis = w_u.sqrt_d_lambda * chol_P  # shape (d, d)

        # Compute predicted measurements at sigma points
        ys = np.zeros((2 * self.d, self.l))
        hat_y = np.asarray(self.h(self._state), dtype=float).ravel()
        for j in range(self.d):
            s_plus = self.phi(self._state, xis[j])
            s_minus = self.phi(self._state, -xis[j])
            ys[j] = np.asarray(self.h(s_plus), dtype=float).ravel()
            ys[self.d + j] = np.asarray(self.h(s_minus), dtype=float).ravel()

        # Predicted measurement mean
        y_bar = w_u.wm * hat_y + w_u.wj * np.sum(ys, axis=0)
