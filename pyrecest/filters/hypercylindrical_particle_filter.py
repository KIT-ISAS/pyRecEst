...
class HypercylindricalParticleFilter(LinPeriodicParticleFilter):
    def __init__(self, n_particles, bound_dim, lin_dim):
        ...
        """
        LinPeriodicParticleFilter.__init__(self, HypercylindricalDiracDistribution(bound_dim, np.zeros((n_particles, bound_dim + lin_dim))))
    ...
    def filter_state(self, new_state):
        ...
        if not isinstance(new_state, HypercylindricalDiracDistribution):
            state_to_set = HypercylindricalDiracDistribution.from_distribution(new_state, np.size(self.filter_state.w))
        else:
    ...
    def predict_nonlinear(self, f, noise_distribution=None, function_is_vectorized=True):
        ...
        self.filter_state.d[0:self.filter_state.bound_dim, :] = np.mod(self.filter_state.d[0:self.filter_state.bound_dim, :], 2 * np.pi) # noqa: E203