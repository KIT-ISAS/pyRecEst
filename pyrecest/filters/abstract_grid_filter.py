...
class AbstractGridFilter(AbstractFilterType):
    ...
    def filter_state(self, new_state: AbstractManifoldSpecificDistribution):
        if not isinstance(new_state, AbstractGridDistribution):
            warnings.warn("new_state is not a GridDistribution. Transforming the distribution with a number of coefficients equal to that of the filter.", RuntimeWarning)
            new_state = self.filter_state.from_distribution(new_state, self.filter_state.grid_values.shape[0], self.filter_state.enforce_pdf_nonnegative)
    ...
    def update_nonlinear(self, likelihood, z):
        grid_vals_new = self.filter_state.grid_values * likelihood(z, self.filter_state.get_grid())
