...
def scenario_database(scenario, scenario_customization_params=None):
    ...
    if scenario == "R2randomWalk":
        ...
        scenario_param["initial_prior"] = GaussianDistribution([0, 0], 0.5 * np.eye(2))
        scenario_param["meas_noise"] = GaussianDistribution([0, 0], 0.5 * np.eye(2))
        scenario_param["sys_noise"] = GaussianDistribution([0, 0], 0.5 * np.eye(2))
        ...
        warnings.warn('Scenario not recognized. Assuming scenario_customization_params contains all parameters.')
