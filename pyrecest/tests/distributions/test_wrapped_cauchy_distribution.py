...
class WrappedCauchyDistributionTest(unittest.TestCase):
    ...
    def test_pdf(self):
        ...
        np.testing.assert_allclose(dist.pdf(xs=self.xs), custom_wrapped.pdf(xs=self.xs), atol=0.0001)

    def test_cdf(self):
        dist = WrappedCauchyDistribution(self.mu, self.gamma)
        np.testing.assert_allclose(dist.cdf(np.array([1])), dist.integrate(np.array([0, 1])))
