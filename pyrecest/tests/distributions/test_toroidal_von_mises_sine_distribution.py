...
class ToroidalVMSineDistributionTest(unittest.TestCase):
    ...
    def test_trigonometric_moment_numerical(self):
        np.testing.assert_almost_equal(self.tvm.trigonometric_moment_numerical(0), np.array([1, 1]), decimal=5)

    def _unnormalized_pdf(self, x):
        return np.exp(self.kappa[0] * np.cos(x[0] - self.mu[0]) + self.kappa[1] * np.cos(x[1] - self.mu[1]) +
    ...
    def test_pdf(self, x):
        ...
        pdf = lambda x: self._unnormalized_pdf(x) * C
