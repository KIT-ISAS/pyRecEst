...
class TestHypercylindricalPartiallyWrappedNormalDistribution(unittest.TestCase):
    def test_pdf(self):
        mu = np.array([5, 1])
        C = np.array([[2, 1], [1, 1]])
        dist = HypercylindricalPartiallyWrappedNormalDistribution(mu, C, 1)
    ...
    def test_hybrid_mean_4d(self):
        ...
        dist = HypercylindricalPartiallyWrappedNormalDistribution(mu, C, 2)
        np.testing.assert_allclose(dist.hybrid_mean(), mu)
