...
class TestANEES(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.uncertainties = np.array([[[1, 0.5], [0.5, 2]],
                                       [[1, 0], [0, 1]],
                                       [[0.5, 0], [0, 1.5]]])
    ...
    def test_ANEES_is_close_to_one(self):
        ...
        for i in range(len(self.groundtruths)):
            samples_for_i = np.random.multivariate_normal(mean=self.groundtruths[i], 
        ...
        np.testing.assert_almost_equal(computed_ANEES, self.groundtruths.shape[-1], decimal=2)
