...
class TestFourierDistribution(unittest.TestCase):
        ...
        for mult_by_n in [True, False]:
            for transformation in ['identity', 'sqrt']:
                ...
                fd = FourierDistribution.from_distribution(
                    dist, n=31, transformation=transformation, store_values_multiplied_by_n=mult_by_n)
                np.testing.assert_array_almost_equal(dist.pdf(xs), fd.pdf(xs))
                fd_real = fd.to_real_fd()
                np.testing.assert_array_almost_equal(
                    dist.pdf(xs), fd_real.pdf(xs))
    ...
    def test_integral_numerical(self):
        ...
        for mult_by_n in [True, False]:
            for transformation in ['identity']:  # ,'sqrt']:
                dist = VMDistribution(
                    np.array(1.5, dtype=np.float32), np.array(2.5, dtype=np.float32))
                fd = FourierDistribution.from_distribution(
                    dist, n=31, transformation=transformation, store_values_multiplied_by_n=mult_by_n)
                np.testing.assert_array_almost_equal(
                    fd.integral_numerical(), 1)
                fd_real = fd.to_real_fd()
                np.testing.assert_array_almost_equal(
                    fd_real.integral_numerical(), 1)
                ...
                np.testing.assert_array_almost_equal(
                    fd_unnorm.integral_numerical(), expected_val)
                fd_unnorm_real = fd_unnorm.to_real_fd()
                np.testing.assert_array_almost_equal(
                    fd_unnorm_real.integral_numerical(), expected_val)
        ...
        for mult_by_n in [True, False]:
            for transformation in ['identity', 'sqrt']:
                ...
                if transformation == 'identity':
                    ...
                    expected_val = (scale_by)**2
                np.testing.assert_array_almost_equal(
                    fd_unnorm.integral(), expected_val)
                fd_unnorm_real = fd_unnorm.to_real_fd()
                np.testing.assert_array_almost_equal(
                    fd_unnorm_real.integral(), expected_val)
                ...
                np.testing.assert_array_almost_equal(
                    fd_norm_real.integral(), 1)

    def test_distance(self):
        dist1 = VMDistribution(
            np.array(0.0, dtype=np.float32), np.array(1.0, dtype=np.float32))
        dist2 = VMDistribution(
        ...
        for mult_by_n in [False, True]:
            fd1 = FourierDistribution.from_distribution(
                dist1, n=31, transformation='sqrt', store_values_multiplied_by_n=mult_by_n)
            fd2 = FourierDistribution.from_distribution(
                dist2, n=31, transformation='sqrt', store_values_multiplied_by_n=mult_by_n)
            hel_like_distance, _ = integrate.quad(lambda x: (np.sqrt(dist1.pdf(np.array(
                x).reshape(1, -1))) - np.sqrt(dist2.pdf(np.array(x).reshape(1, -1))))**2, 0, 2*np.pi)
            fd_diff = fd1 - fd2
            np.testing.assert_array_almost_equal(
                fd_diff.integral(), hel_like_distance)
