...
class UnscentedKalmanFilterTest(unittest.TestCase):
    ...
    def test_update_with_meas_noise_and_meas_1d(self):
        kf = UnscentedKalmanFilter(GaussianDistribution(array([0.0]), array([[1.0]])))
        kf.update_identity(array(4.0), array(1.0))
        npt.assert_allclose(kf.get_estimate().C, 0.5)
        npt.assert_allclose(kf.get_point_estimate(), 2.0)
    ...
    def test_predict_identity_1d(self):
        kf = UnscentedKalmanFilter(GaussianDistribution(array([0]), array([[1]])))
    ...
    def test_predict_linear_2d(self):
        ...
        self.assertTrue(np.allclose(kf.get_point_estimate(), array([0.0, 2.0])))
        self.assertTrue(np.allclose(kf.filter_state.covariance(), diag(array([3.0, 9.0]))))
        kf.predict_linear(diag(array([1.0, 2.0])), diag(array([2.0, 1.0])), array([2.0, -2.0]))
        self.assertTrue(np.allclose(kf.get_point_estimate(), array([2.0, 2.0])))
