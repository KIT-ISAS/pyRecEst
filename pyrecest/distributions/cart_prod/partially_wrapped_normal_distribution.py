...
class PartiallyWrappedNormalDistribution(AbstractHypercylindricalDistribution):
    ...
    def pdf(self, xs, m=3):
        ...
        for start in range(0, n, batch_size):
            ...
            # Expand batch and offsets for vectorized addition
            # Shape after tiling: (batch_size, num_offsets, bound_dim)
            if self.bound_dim > 0:
                wrapped_periodic = (
                    batch_wrapped[:, : self.bound_dim, None]
                    + offset_array.T[:, None, :]
                )  # noqa: E203
                # Transpose to shape: (batch_size, num_offsets, bound_dim)
                wrapped_periodic = wrapped_periodic.transpose(1, 0, 2)
                # Reshape to (batch_size * num_offsets, bound_dim)
