...
class AbstractHypercylindricalDistribution(AbstractLinPeriodicCartProdDistribution):
    ...
    def integrate_numerically(self, integration_boundaries=None):
        ...
        def f(*args):
            return self.pdf(array(args)).item()
    ...
    def get_reasonable_integration_boundaries(self, scalingFactor=10):
        ...
        boundaries = []
        # Periodic dimensions: [0, 2*pi]
        for _ in range(self.bound_dim):
            boundaries.append([0.0, 2.0 * pi])
        # Linear dimensions: [mean - scale*sigma, mean + scale*sigma]
        for i in range(self.lin_dim):
            sigma = sqrt(P[i, i])
            boundaries.append(
                [
                    m[self.bound_dim + i] - scalingFactor * sigma,
                    m[self.bound_dim + i] + scalingFactor * sigma,
                ]
            )
