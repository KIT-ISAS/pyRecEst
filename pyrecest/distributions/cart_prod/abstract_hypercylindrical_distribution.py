...
class AbstractHypercylindricalDistribution(AbstractLinPeriodicCartProdDistribution):
    ...
    def condition_on_linear(self, input_lin, normalize=True):
        ...
        """
        assert np.size(input_lin) == self.lin_dim and np.ndim(input_lin)<=1, 'Input should be of size (lin_dim,).'
    ...
    def condition_on_periodic(self, input_periodic, normalize=True):
        ...
        """
        assert np.size(input_periodic) == self.bound_dim and np.ndim(input_periodic)<=1, 'Input should be of size (lin_dim,).'
    ...
    def linear_mean_numerical(self):
        ...
        if self.lin_dim == 1 and self.bound_dim == 1:
            mu = scipy.integrate.nquad(lambda x, y: y * self.pdf([x, y]), [[0, 2*np.pi], [-np.inf, np.inf]])[0]
        elif self.bound_dim == 2 and self.lin_dim == 1:
            mu = scipy.integrate.nquad(lambda x, y, z: z * self.pdf([x, y, z]), [[0, 2*np.pi], [0, 2*np.pi], [-np.inf, np.inf]])[0]
        elif self.bound_dim == 1 and self.lin_dim == 2:
            mu = np.empty(2)
            mu[0] = scipy.integrate.nquad(lambda x, y, z: y * self.pdf([x, y, z]), [[0, 2*np.pi], [-np.inf, np.inf], [-np.inf, np.inf]])[0]
            mu[1] = scipy.integrate.nquad(lambda x, y, z: z * self.pdf([x, y, z]), [[0, 2*np.pi], [-np.inf, np.inf], [-np.inf, np.inf]])[0]
        else:
    ...
    def mode_numerical(self, starting_point=None):
        ...
        if starting_point is None:
            starting_point = np.concatenate([np.pi * np.ones((self.bound_dim)), np.zeros((self.lin_dim))])

        bounds = [(0, 2*np.pi) if i < self.bound_dim else (-np.inf, np.inf) for i in range(len(starting_point))]
        
        m = scipy.optimize.minimize(lambda x: -self.pdf(x), starting_point, bounds=bounds).x
