...
class ToroidalVonMisesMatrixDistribution(AbstractToroidalDistribution):
    ...
    def norm_const_approx(self, n=8):
        ...
        if n >= 2:
            total += (a11 ** 2 + a12 ** 2 + a21 ** 2 + a22 ** 2 + 2 * k1 ** 2 + 2 * k2 ** 2) * np.pi ** 2 / factorial(2)  # n=2

        if n >= 3:
            total += (6 * a11 * k1 * k2 * np.pi ** 2) / factorial(3)  # n=3

        if n >= 4:
            total += (3.0 / 16) * (3 * a11 ** 4 + 3 * a12 ** 4 + 3 * a21 ** 4 + 8 * a11 * a12 * a21 * a22 + 6 * a21 ** 2 * a22 ** 2 + 3 * a22 ** 4 + 8 * a21 ** 2 * k1 ** 2 + 8 * a22 ** 2 * k1 ** 2 + 8 * k1 ** 4 + 8 * (3 * a21 ** 2 + a22 ** 2 + 4 * k1 ** 2) * k2 ** 2 + 8 * k2 ** 4 + 2 * a11 ** 2 * (3 * a12 ** 2 + 3 * a21 ** 2 + a22 ** 2 + 12 * (k1 ** 2 + k2 ** 2)) + 2 * a12 ** 2 * (a21 ** 2 + 3 * a22 ** 2 + 4 * (3 * k1 ** 2 + k2 ** 2))) * np.pi ** 2 / factorial(4)  # n=4

        if n >= 5:
            total += (15.0 / 4) * np.pi ** 2 * k1 * k2 * (3 * a11 ** 3 + 3 * a11 * a12 ** 2 + 3 * a11 * a21 ** 2 + a11 * a22 ** 2 + 4 * a11 * k1 ** 2 + 4 * a11 * k2 ** 2 + 2 * a12 * a21 * a22) / factorial(5)  # n=5

        if n >= 6:
            total += (5.0 / 64) * np.pi ** 2 * (
        ...
        if n >= 8:
            ...
            total += (35.0 / 4096) * np.pi ** 2 * (part1 + part2 + part3 + part4 + part5) / factorial(8)  # n=8
    ...
    def log_likelihood(self, samples):
        ...
        exponent = (
            self.kappa[0] * np.cos(samples[:, 0] - self.mu[0]) +
            self.kappa[1] * np.cos(samples[:, 1] - self.mu[1]) +
            np.cos(samples[:, 0] - self.mu[0]) * self.A[0, 0] * np.cos(samples[:, 1] - self.mu[1]) +
            np.cos(samples[:, 0] - self.mu[0]) * self.A[0, 1] * np.sin(samples[:, 1] - self.mu[1]) +
            np.sin(samples[:, 0] - self.mu[0]) * self.A[1, 0] * np.cos(samples[:, 1] - self.mu[1]) +
            np.sin(samples[:, 0] - self.mu[0]) * self.A[1, 1] * np.sin(samples[:, 1] - self.mu[1])
        )
        return len(samples) * np.log(self.C) + np.sum(exponent)
    ...
    def log_likelihood_diff(self, samples):
        ...
        ld[0] = np.sum(
            self.kappa[0] * np.sin(samples[:, 0] - self.mu[0]) +
            np.sin(samples[:, 0] - self.mu[0]) * self.A[0, 0] * np.cos(samples[:, 1] - self.mu[1]) +
            np.sin(samples[:, 0] - self.mu[0]) * self.A[0, 1] * np.sin(samples[:, 1] - self.mu[1]) -
            np.cos(samples[:, 0] - self.mu[0]) * self.A[1, 0] * np.cos(samples[:, 1] - self.mu[1]) -
            np.cos(samples[:, 0] - self.mu[0]) * self.A[1, 1] * np.sin(samples[:, 1] - self.mu[1])
        )

        # derivative w.r.t. mu2
        ld[1] = np.sum(
            self.kappa[1] * np.sin(samples[:, 1] - self.mu[1]) +
            np.cos(samples[:, 0] - self.mu[0]) * self.A[0, 0] * np.sin(samples[:, 1] - self.mu[1]) -
            np.cos(samples[:, 0] - self.mu[0]) * self.A[0, 1] * np.cos(samples[:, 1] - self.mu[1]) +
            np.sin(samples[:, 0] - self.mu[0]) * self.A[1, 0] * np.sin(samples[:, 1] - self.mu[1]) -
            np.sin(samples[:, 0] - self.mu[0]) * self.A[1, 1] * np.cos(samples[:, 1] - self.mu[1])
        )

        # derivative w.r.t. kappa1
        ld[2] = -n * self.C * cinv_diff[0] + np.sum(np.cos(samples[:, 0] - self.mu[0]))

        # derivative w.r.t. kappa2
        ld[3] = -n * self.C * cinv_diff[1] + np.sum(np.cos(samples[:, 1] - self.mu[1]))

        # derivative w.r.t. a11
        ld[4] = -n * self.C * cinv_diff[2] + np.sum(np.cos(samples[:, 0] - self.mu[0]) * np.cos(samples[:, 1] - self.mu[1]))

        # derivative w.r.t. a12
        ld[5] = -n * self.C * cinv_diff[3] + np.sum(np.cos(samples[:, 0] - self.mu[0]) * np.sin(samples[:, 1] - self.mu[1]))

        # derivative w.r.t. a21
        ld[6] = -n * self.C * cinv_diff[4] + np.sum(np.sin(samples[:, 0] - self.mu[0]) * np.cos(samples[:, 1] - self.mu[1]))

        # derivative w.r.t. a22
        ld[7] = -n * self.C * cinv_diff[5] + np.sum(np.sin(samples[:, 0] - self.mu[0]) * np.sin(samples[:, 1] - self.mu[1]))
    ...
    def marginalize_to_1D(self, dimension):
        ...
        def alpha(x):
            return (self.kappa[other - 1] +
                    np.cos(x - self.mu[dimension - 1]) * self.A[0, 0] +
                    np.sin(x - self.mu[dimension - 1]) * self.A[1, 0])

        def beta(x):
            return (np.sin(x - self.mu[dimension - 1]) * self.A[1, 1] +
                    np.cos(x - self.mu[dimension - 1]) * self.A[0, 1])

        def f(x):
            return (2 * np.pi * self.C *
                    iv(0, np.sqrt(alpha(x) ** 2 + beta(x) ** 2)) *
                    np.exp(self.kappa[dimension - 1] * np.cos(x - self.mu[dimension - 1])))
    ...
    def shift(self, shift_angles):
        ...
        """
        shift_angles = np.reshape(shift_angles, -1)
    ...
    def mle_numerical(samples, method="nelder-mead"):
        ...
        def neg_log_likelihood(x):
            mu_temp = x[:2]
            kappa_temp = x[2:4]
            A_temp = x[4:8].reshape(2, 2)
            if np.all(kappa_temp > 0):
                dist = ToroidalVonMisesMatrixDistribution(mu_temp, kappa_temp, A_temp)
                return -dist.log_likelihood(samples)
            return np.inf

        # start value: use mean direction and zero correlation
        mean_dir = np.arctan2(np.mean(np.sin(samples), axis=0), np.mean(np.cos(samples), axis=0))
        mean_cos = np.mean(np.cos(samples - mean_dir[np.newaxis, :]), axis=0)
        kappa_start = np.clip(-np.log(np.clip(mean_cos, 1e-10, 1 - 1e-10)), 0.01, 10.0)
        start_value = np.concatenate([mean_dir, kappa_start, np.zeros(4)])

        result = scipy.optimize.minimize(neg_log_likelihood, start_value, method=method,
                                         options={"xatol": 1e-6, "fatol": 1e-6, "maxiter": 10000})
        x = result.x
        return ToroidalVonMisesMatrixDistribution(x[:2], x[2:4], x[4:8].reshape(2, 2))
