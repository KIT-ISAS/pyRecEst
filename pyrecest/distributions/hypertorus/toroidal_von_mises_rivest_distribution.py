...
class ToroidalVMRivestDistribution:
    def __init__(self, mu_, kappa_, alpha_, beta_):
        assert mu_.shape == (2, 1)
        assert kappa_.shape == (2, 1)
    ...
    def to_toroidal_vm_matrix_distribution(self):
        tvm = ToroidalVMMatrixDistribution(self.mu, self.kappa, np.array([[0, 0], [0, self.beta]]))
        return tvm

    def to_toroidal_vm_rivest_distribution(self):
        tvm = ToroidalVMRivestDistribution(self.mu, self.kappa, 0, self.beta)
        return tvm

    def to_toroidal_wn(self):
        if self.beta ** 2 >= self.kappa[0] * self.kappa[1]:
            print("Warning: conversion may not be meaningful")
    ...
    def shift(self, shift_by):
        assert shift_by.shape == (self.dim, 1)
        tvm = self
