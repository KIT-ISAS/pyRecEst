...
from pyrecest.backend import all, array, shape, linalg, eye, equal, reshape, zeros_like, log, mod, pi, pad, fft, exp, stack, conj, zeros, real, imag, ones, linspace, sqrt, ndim, meshgrid, column_stack, sum, atleast_1d, prod
...
class HypertoroidalFourierDistribution(AbstractHypertoroidalDistribution, AbstractOrthogonalBasisDistribution):
    ...
    def transform_via_coefficients(self, desired_transformation, n_coefficients=None):
        if n_coefficients is None:
            n_coefficients = [self.coeff_mat.shape[i] for i in range(self.dim)]
        ...
        if desired_transformation == 'identity':
            ...
            transformed_instance = HypertoroidalFourierDistribution(self.coeff_mat, new_trans)
            
            # Perform the transformation (square)
            transformed_instance.coeff_mat = scipy.ndimage.convolve(self.coeff_mat, self.coeff_mat, 'constant', cval=0) 
    ...
    def truncate(self, n_coefficients, forceNormalization=False):
        ...
        if equal(result.coeff_mat.shape, n_coefficients):  # Do not need to truncate as already correct size
