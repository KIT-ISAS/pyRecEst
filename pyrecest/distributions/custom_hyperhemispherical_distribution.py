...
class CustomHyperhemisphericalDistribution(CustomDistribution, AbstractHyperhemisphericalDistribution):
    def __init__(self, f, dim, shift_by: np.ndarray = None):
        ...
        assert shift_by.shape[0] == self.dim + 1, "Shift_by vector length must match the dimension."
        self.shift_by = shift_by
        
    def pdf(self, xs):
        assert xs.shape[-1] == self.dim + 1
        # Reshape to properly handle the (d,) shape as well as the (n, d) case
        p = self.scale_by * self.f(np.reshape(xs, (-1, xs.shape[-1])) - self.shift_by[np.newaxis, :])
