...
class FourierDistribution:
    ...
    def __sub__(self, other):
        ...
        assert self.a is not None and other.a is not None or self.c is not None and other.c is not None
    ...
    def pdf(self, xs):
        ...
        p = a[0] / 2 + np.sum(a[1:].reshape(1, -1) * np.cos(xs * range) + b.reshape(1, -1) * np.sin(xs * range), axis=1)
    ...
    def normalize(self):
        ...
        if self.a is not None and self.b is not None:
            ...
            if self.transformation == 'identity':
                ...
                a_new = self.a / np.sqrt(a0_non_rooted * pi)
                b_new = self.b / np.sqrt(a0_non_rooted * pi)
            ...
            if self.transformation == 'identity':
                ...
                c_norm_div_by_sqrt2 = np.linalg.norm(np.concatenate(((1 / np.sqrt(2)) * c[0].reshape(1), c[1:]), axis=-1))
                c_new = self.c / c_norm_div_by_sqrt2 * (1 / (2 * np.sqrt(pi)))
    ...
    def integral(self):
        ...
        if self.a is not None and self.b is not None:
            ...
            if self.transformation == 'identity':
                ...
                from_c1_to_end = np.sum((np.real(c[1:])) ** 2) + np.sum((np.imag(c[1:])) ** 2)
    ...
    def integral_numerical(self):
        ...
        integral, _ = integrate.quad(lambda x: self.pdf(np.array(x).astype(a.dtype).reshape(1, -1)), 0, 2 * pi)
        return integral
    def plot_grid(self):
    ...
    def get_a_b(self):
        ...
        assert self.n is None or (a.size + b.size) == self.n  # Other case not implemented yet!
        return a, b
    def get_c(self):
        if self.a is not None:
    ...
    def from_distribution(dist, n, transformation, store_values_multiplied_by_n=True):
        if isinstance(dist, WDDistribution):
            fd = FourierDistribution(np.conj(dist.trigonometric_moment(n, whole_range=True)) / (2 * np.pi), transformation, multiplied_by_n=False)
            if store_values_multiplied_by_n:
                raise Warning('Scaling up for WD (this is not recommended).')
                fd.c = fd.c * fd.n
        else:
