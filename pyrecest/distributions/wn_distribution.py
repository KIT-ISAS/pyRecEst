...
class WNDistribution:
    ...
    def cdf(self, xs, startingPoint=0, n=10):
        ...
        def ncdf(from_, to): return 1 / 2 * (erf((self.mu - from_) / (np.sqrt(2)
                                                                      * self.sigma)) - erf((self.mu - to) / (np.sqrt(2) * self.sigma)))
        val = ncdf(startingPoint, xs)
        for i in range(1, n + 1):
            val = val + ncdf(startingPoint + 2 * np.pi * i, xs + 2 * np.pi * i) + \
