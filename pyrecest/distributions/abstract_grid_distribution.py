...
class AbstractGridDistribution(AbstractDistributionType):
    ...
    @beartype
    def __init__(self, grid_values, grid_type: str = "custom", grid = None, dim=None, enforce_pdf_nonnegative: bool =True):
        assert not grid_type == "custom" or grid is not None # if grid_type is custom, grid needs to be given
        assert grid is None or np.size(grid) == 0 or grid.shape[0] == grid_values.shape[0]
        assert grid is None or np.size(grid) == 0 or grid.ndim==1 or grid.shape[1] == dim
        if grid is None or grid.ndim>1 and grid.shape[0] < grid.shape[1]:
            warnings.warn("Warning: Dimension is higher than number of grid points. Verify that this is really intended.")
    ...
    def integrate(self, integration_boundaries = None):
        assert integration_boundaries is None, "Custom integration boundaries are currently not supported"
    ...
    def normalize_in_place(self, tol=1e-4, warn_unnorm=True):
        ...
        if np.any(self.grid_values < 0):
            warnings.warn('Warning: There are negative values. This usually points to a user error.')
        elif np.abs(int_val) < 1e-200:
            raise ValueError('Sum of grid values is too close to zero, this usually points to a user error.')
        elif np.abs(int_val - 1) > tol:
            if warn_unnorm:
                warnings.warn('Warning: Grid values apparently do not belong to a normalized density. Normalizing...')
        else:
            return
