...
class AbstractParticleFilter(AbstractFilter):
    ...
    def update_identity(self, noise_distribution, z, shift_instead_of_add=True):
        ...
        if not shift_instead_of_add:
            likelihood = LikelihoodFactory.additive_noise_likelihood(lambda x: x, noise_distribution)
    ...
    def update_nonlinear(self, likelihood, z=None):
        ...
        self.dist.d = self.dist.sample(self.dist.d.shape[1])
        self.dist.w = np.full(self.dist.d.shape[1], 1 / self.dist.d.shape[1])
