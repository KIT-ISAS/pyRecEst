...
class AbstractMixture(AbstractDistributionType):
    ...
    def _sample_component(self, component_index: int, n_samples: int):
        try:
    ...
    def sample(self, n: Union[int, int32, int64]):
        ...
        if pyrecest.backend.__backend_name__ == "jax":
            samples = [
                self._sample_component(int(component_index), 1)
        ...
        for component_index in range(len(self.dists)):
            sample_positions = np.flatnonzero(component_indices == component_index)
            occ_val = int(sample_positions.size)
            if occ_val != 0:
                sample_i = self._sample_component(component_index, occ_val)
                s[array(sample_positions, dtype=int64)] = sample_i
