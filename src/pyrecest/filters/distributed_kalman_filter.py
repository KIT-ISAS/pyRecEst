...
class LinearGaussianInformationContribution:
    ...
    The contribution corresponds to

    ``H.T @ inv(R) @ H`` and ``H.T @ inv(R) @ z``

    for a measurement model ``z = H x + v`` with ``v ~ N(0, R)``.
    Contributions from conditionally independent measurements can be summed and
    applied to each node's local prior.
    """
...
def _as_information_contribution(contribution):
    ...
    if information_matrix.shape != (state_dim, state_dim):
        raise ValueError(
            "information_matrix must have shape "
            "(state_dim, state_dim), where state_dim is the length of "
            "information_vector"
        )
...
class DistributedKalmanFilter:
    ...
    The class manages one local :class:`KalmanFilter` per node. Each node holds
    its own Gaussian belief over the same Euclidean state vector. Local
    measurements are converted to additive information-form contributions,
    summed, and applied to the selected node priors.

    This class intentionally does not model network topology, consensus
    ...
    are available to the network at a given filtering step.
    """
    ...
    def __init__(self, initial_states, num_nodes=None):
        ...
        Parameters
        ----------
        initial_states : GaussianDistribution, tuple, or sequence
            Either one initial state to use for one node, one initial state to
            replicate when ``num_nodes`` is provided, or a sequence with one
            initial state per node. Single states follow the same format as
            :class:`KalmanFilter`: either a GaussianDistribution or a
            ``(mean, covariance)`` tuple.
        num_nodes : int, optional
            Number of nodes. If given together with a single initial state, the
            state is replicated for all nodes. If given together with a
            sequence, the sequence length must match ``num_nodes``.
        """
        if num_nodes is not None:
            if int(num_nodes) < 1:
                raise ValueError("num_nodes must be at least 1")
            if _is_single_state(initial_states):
                states = [initial_states] * int(num_nodes)
            else:
                states = list(initial_states)
                if len(states) != int(num_nodes):
                    raise ValueError(
                        "initial_states must contain num_nodes entries "
                        "when a sequence of states is provided"
                    )
    ...
    @staticmethod
    def local_information_contribution(
        measurement,
        measurement_matrix,
        meas_noise,
    ):
        ...
        measurement : array-like, shape (m,)
            Local measurement vector ``z``.
        measurement_matrix : array-like, shape (m, n)
            Linear measurement matrix ``H``.
        ...
        information_matrix = (
            transpose(measurement_matrix) @ inv_noise_times_matrix
        )
        information_vector = (
            transpose(measurement_matrix) @ inv_noise_times_measurement
        )
        return LinearGaussianInformationContribution(
            _symmetrize(information_matrix),
            information_vector,
    ...
    @staticmethod
    def aggregate_information_contributions(
        contributions,
        *,
        state_dim=None,
    ):
        ...
        for contribution in contributions:
            ...
            if contribution.information_matrix.shape != (state_dim, state_dim):
                raise ValueError("all information matrices must have shape (state_dim, state_dim)")
        ...
        return LinearGaussianInformationContribution(
            _symmetrize(total_matrix),
            total_vector,
        )

    @staticmethod
    def _posterior_from_information_contribution(
        prior_state,
        contribution,
    ):
        contribution = _as_information_contribution(contribution)
        ...
        if contribution.information_vector.shape[0] != state_dim:
            raise ValueError(
                "information contribution dimension must match the node state dimension"
            )
        ...
        posterior_information_vector = (
            prior_information_vector + contribution.information_vector
        )

        posterior_mean = linalg.solve(
            posterior_precision,
            posterior_information_vector,
        )
    ...
    def update_from_information_contribution(
        self,
        contribution,
        *,
        node_indices=None,
    ):
        """Apply one already aggregated information contribution to nodes."""
        contribution = _as_information_contribution(contribution)
        if contribution.information_vector.shape[0] != self.dim:
            raise ValueError(
                "information contribution dimension must match the common state dimension"
            )
    ...
    def update_from_information_contributions(
        self,
        contributions,
        *,
        node_indices=None,
        ...
        contribution = self.aggregate_information_contributions(
            contributions,
            state_dim=self.dim,
        )
        self.update_from_information_contribution(
            contribution,
    ...
    def update_distributed_linear(
        ...
        *,
        ...
        ``local_measurements`` is a sequence of local measurement vectors.
        ``measurement_matrices`` and ``meas_noises`` may either contain one
        entry per measurement or a single shared entry that is reused for all
        measurements.
        """
        ...
        meas_noises = _broadcast_to_length(
            meas_noises,
            len(local_measurements),
            "meas_noises",
        )
        ...
        contributions = [
            self.local_information_contribution(
                measurement,
                measurement_matrix,
                meas_noise,
            )
