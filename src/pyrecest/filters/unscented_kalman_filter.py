...
class UnscentedKalmanFilter(AbstractFilter, EuclideanFilterMixin):
    ...
    def update_identity(
        ...
        *,
        ...
        first_arg_shape = getattr(atleast_1d(meas_noise), "shape", None)
        second_arg_shape = getattr(atleast_1d(measurement), "shape", None)
        if first_arg_shape != cov_shape and second_arg_shape == cov_shape:
            warnings.warn(
