...
class IdkfNode(AbstractFilter, EuclideanFilterMixin):
    ...
    def __init__(
        ...
        *,
        contribution_bank: Mapping[ContributionKey, InformationContribution] | None = None,
        ...
        self.measurement_matrix = None if measurement_matrix is None else atleast_2d(measurement_matrix)
        self.meas_noise = None if meas_noise is None else atleast_2d(meas_noise)
        self.measurement_models = measurement_models
        self.local_filter_bank = {} if local_filter_bank is None else dict(local_filter_bank)
        ...
        if contribution_bank is not None:
            for key, contribution in contribution_bank.items():
                if key != contribution.key:
                    raise ValueError('contribution_bank keys must match contribution.key')
    ...
    def _coerce_gaussian(state) -> GaussianDistribution:
        if isinstance(state, GaussianDistribution):
            return GaussianDistribution(atleast_1d(state.mu), atleast_2d(state.C), check_validity=False)
        if isinstance(state, tuple) and len(state) == 2:
            mean, covariance = state
            return GaussianDistribution(atleast_1d(mean), atleast_2d(covariance), check_validity=False)
        raise ValueError('state must be a GaussianDistribution or a (mean, covariance) tuple')

    @staticmethod
    def _sum_precision_matrices(covariances):
        iterable = covariances.values() if isinstance(covariances, Mapping) else covariances
        total = None
    ...
    def current_contribution_keys(self) -> frozenset[ContributionKey]:
        return frozenset(
            key for key, contribution in self.contribution_bank.items()
    ...
    def filter_state(self) -> GaussianDistribution:
        ...
        return GaussianDistribution(linalg.solve(Y, y_sum), linalg.inv(Y), check_validity=False)
    ...
    def predict_linear(
        ...
        *,
        ...
        for contribution in self.contribution_bank.values():
            if not self.is_contribution_current(contribution):
                raise ValueError('Cannot predict with stale IDKF contributions in the bank')
        ...
        operation_hash = _extend_hash(self.global_information_state.operation_hash, 'predict_linear')
        ...
        for contribution in self.contribution_bank.values():
            ...
            predicted_component = A @ linalg.solve(Y_old, contribution.y) + explicit_input
        ...
        if state.transform_to_end is not None:
            state.transform_to_end = state.transform_to_end @ Y_old @ linalg.inv(A) @ predicted_covariance
    ...
    def update_linear(
        ...
        *,
        ...
        key = self.own_contribution_key if contribution_key is None else self._coerce_contribution_key(contribution_key)
        ...
        all_models = measurement_models if measurement_models is not None else self.measurement_models
    ...
    def update_model(self, measurement_model, measurement, **kwargs):
        measurement_matrix, meas_noise = self._measurement_model_parts(measurement_model)
        self.update_linear(measurement, measurement_matrix, meas_noise, **kwargs)

    def export_contribution(self, key: ContributionKey | Hashable | None = None) -> InformationContribution:
        contribution_key = self.own_contribution_key if key is None else self._coerce_contribution_key(key)
    ...
    def drop_stale_contributions(self, *, keep_own: bool = True) -> frozenset[ContributionKey]:
        dropped = set()
    ...
    def _input_for_contribution(
        ...
        if sys_input is not None and not assume_zero_remote_inputs:
            raise ValueError('Provide input_by_node or set assume_zero_remote_inputs=True for remote contributions')
        return zeros((self.dim,))

    def _resolve_measurement_model(self, measurement_matrix, meas_noise):
        H = self.measurement_matrix if measurement_matrix is None else measurement_matrix
    ...
    def _measurement_model_parts(model):
        ...
        if isinstance(model, Mapping):
            ...
            R = model.get('meas_noise', model.get('measurement_noise_cov', model.get('R')))
            if H is None or R is None:
                raise AttributeError('Measurement model mappings must expose H/measurement_matrix and R/meas_noise')
            return atleast_2d(H), atleast_2d(R)
        H = _required(model, 'measurement_matrix', 'H')
        R = _required(model, 'meas_noise', 'measurement_noise_cov', 'R')
        return atleast_2d(H), atleast_2d(R)

    def _coerce_contribution_key(self, key: ContributionKey | Hashable) -> ContributionKey:
    ...
    def _assert_contribution_compatible(self, contribution: InformationContribution):
        ...
        if contribution.operation_count != state.operation_count:
            raise ValueError('Cannot fuse IDKF contributions with different operation counts')
        if contribution.operation_hash != state.operation_hash:
            raise ValueError('Cannot fuse IDKF contributions with different operation histories')

    def _assert_global_information_compatible(self, other_state: GlobalInformationState):
        ...
        if not allclose(
            ...
            raise ValueError('Cannot fuse IDKF nodes with different global information matrices')
