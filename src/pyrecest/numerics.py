...
def is_symmetric(matrix, *, atol: float = 1e-10) -> bool:
    ...
    arr = _to_numpy_array(matrix)
    return bool(arr.ndim == 2 and arr.shape[0] == arr.shape[1] and np.allclose(arr, arr.T, atol=atol, rtol=0.0))
...
def is_positive_semidefinite(matrix, *, atol: float = 1e-10) -> bool:
    ...
    arr = _to_numpy_array(matrix)
    if arr.ndim != 2 or arr.shape[0] != arr.shape[1] or not is_symmetric(arr, atol=atol):
        return False
...
def assert_covariance_matrix(matrix, *, name: str = "covariance", dim: int | None = None, atol: float = 1e-10):
    ...
    if dim is not None and arr.shape[0] != dim:
        raise DimensionMismatchError(f"{name} dimension {arr.shape[0]} does not match expected dimension {dim}.")
    ...
    if not is_positive_semidefinite(arr, atol=atol):
        raise NumericalStabilityError(f"{name} must be positive semidefinite within atol={atol}.")
