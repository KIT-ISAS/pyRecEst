...
class TestMultiSessionAssignment(unittest.TestCase):
    ...
    def _assert_valid_matching(self, selected_edge_mask, left_nodes, right_nodes):
        selected_left_nodes = set()
        selected_right_nodes = set()
        for edge_index, is_selected in enumerate(selected_edge_mask):
            if not bool(is_selected):
                continue
            left_node = int(left_nodes[edge_index])
            right_node = int(right_nodes[edge_index])
            self.assertNotIn(left_node, selected_left_nodes)
            self.assertNotIn(right_node, selected_right_nodes)
            selected_left_nodes.add(left_node)
            selected_right_nodes.add(right_node)
    ...
    def test_sparse_backend_matches_previous_linprog_backend(self):
        ...
        edge_gains = array(
            [0.1 + ((edge_index * 37) % 97) / 10.0 for edge_index in range(40)],
            dtype=float,
        )
