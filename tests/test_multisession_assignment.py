...
class TestMultiSessionAssignment(unittest.TestCase):
    ...
    def _assert_valid_matching(self, mask, left_nodes, right_nodes):
        selected_indices = [idx for idx, is_selected in enumerate(mask) if bool(is_selected)]
        selected_left_nodes = [int(left_nodes[idx]) for idx in selected_indices]
        selected_right_nodes = [int(right_nodes[idx]) for idx in selected_indices]
        self.assertEqual(len(selected_left_nodes), len(set(selected_left_nodes)))
        self.assertEqual(len(selected_right_nodes), len(set(selected_right_nodes)))
    ...
    def test_sparse_backend_matches_previous_linprog_backend(self):
        ...
        selected_pairs = [
            ...
            if (3 * left + 5 * right) % 7 < 2
        ]

        left_nodes = array([left for left, _ in selected_pairs], dtype=int)
        right_nodes = array([right for _, right in selected_pairs], dtype=int)
        edge_gains = array(
            [
                0.1 + ((11 * left + 7 * right) % 97) / 10.0
                for left, right in selected_pairs
            ],
            dtype=float,
        )
        solve_matching = getattr(multisession_assignment_module, "_solve_max_weight_matching")
