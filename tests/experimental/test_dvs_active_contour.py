...
from pyrecest.experimental.dvs import activity_profile, normal_flow_activity, rectangle_contour_samples
...
def test_rectangle_activity_matches_horizontal_translation():
    ...
    by_edge = {edge: float(np.mean(activities[np.array(contour.edge_labels) == edge])) for edge in set(contour.edge_labels)}
