...
from pyrecest.filters import DistributedKalmanFilter, LinearGaussianInformationContribution
...
class DistributedKalmanFilterTest(unittest.TestCase):
    def test_initialization_replicates_single_state(self):
        ...
        self.assertTrue(allclose(dkf.means, array([[0.0, 1.0], [0.0, 1.0], [0.0, 1.0]])))
    ...
    def test_update_from_information_contribution_can_update_selected_nodes(self):
        initial_state = (array([0.0]), array([[2.0]]))
        dkf = DistributedKalmanFilter(initial_state, num_nodes=2)
    ...
    def test_update_distributed_model_matches_distributed_linear(self):
        ...
        filter_model.update_distributed_model(
            measurement_models,
            measurements,
        )
